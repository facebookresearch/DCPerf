...
"""AMD derived-metric functions.

These are shared across Zen generations (many are reused by zen3/zen4/zen5/
zen5es), so unlike the ARM package -- where each CPU had its own namespaced
functions -- the AMD metric functions live together here. Each CPU's module
under ``arches/`` selects the subset it needs and registers its metric list.
Bodies are preserved byte-for-byte from the original generate_amd_perf_report.py.
"""
...
try:
    ...
    from amd_perf.core import get_duration_series, skip_if_missing  # pyre-ignore[21]
