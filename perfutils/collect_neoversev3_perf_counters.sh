...
### (hns_mc_reqs_{local,remote}_sn). Per the CMN-700/S3 TRM, event 0x0D
### ("requests sent to MC") is a REQUEST count partitioned by pmu_sn_home_sel
### (_all / _sn=SN-bound / _home=Home-bound); the TRM assigns it no byte value,
### so "x 64B" is only a proxy. Used here as a bounded MESH MC-REQUEST ESTIMATE
### + local/remote split. We deliberately AVOID the "_all" filter for bandwidth:
### summing the routing-partitioned _all events across both mesh instances
### tallies the same DRAM accesses under multiple filter/mesh combinations, so it
### OVER-READS past the physical DRAM ceiling under load. Even _sn x 64B
### undercounts writes (a request count is not a byte count). We also do not
### derive bandwidth from XP DAT flits (they fire at every mesh crosspoint ->
### multi-hop overcount).
###
