...
* Role `clientserver`:
  ...
  * `base_port` \- Starting port that the HTTP server workers will listen to.
    The range of `[base_port, base_port + nproc)` must be available. Defaults to 8001.
  * `stats_port` \- Load balancer stats port. Defaults to 16667.
* Role `db`:
...
| `--base-port` | Base port for HTTP server workers | `8001` |
