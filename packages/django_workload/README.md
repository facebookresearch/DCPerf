...
* Cassandra DB node:
  ...
  * Thrift backend server ports: `[9091, 9091 + nproc - 1]` (number of ports required configurable via `--thrift-server-workers`)
* Clientserver node:
...
You can also configure the number of Thrift server workers, which defaults to the number of logical cores:
...
- **Cinder** (default): Meta's performance-oriented Python runtime with JIT support
- **CPython**: The standard Python interpreter

As of April 2026, Cinder with JIT can work with ARM CPUs, so we recommend using Cinder+JIT
across the board with the `django_workload_default` job. The separate `django_workload_arm` job
is no longer needed.

To specify which interpreter to use, add the `interpreter` parameter to your command:

```
# Run with Cinder (default)
./benchpress_cli.py run django_workload_default -r standalone

# Run with CPython
./benchpress_cli.py run django_workload_default -r standalone -i '{"interpreter": "cpython"}'

# Run with Cinder without JIT
./benchpress_cli.py run django_workload_default -r standalone -i '{"interpreter": "cinder", "use_jit": 0}'
```
...
When JIT is enabled, the following environment variables are set automatically:
...
**Note**: JIT will not enable when using CPython interpreter regardless of the `use_jit` setting.
...
DjangoBench Mini is a shrunken version of DjangoBench that aims to reduce execution time to less than 30 seconds
and can potentially be used for emulations.  It reuses the dataset generated by the full DjangoBench and has a
shorter runtime because it runs only one iteration with the number of repetitions set to 1000.
...
This will generate the necessary dataset and store it in `/data/cassandra/data`.
Please create a backup of this folder to avoid data loss.
