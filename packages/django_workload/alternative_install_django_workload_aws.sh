...
sudo yum install -y memcached libmemcached-devel zlib-devel screen \
...
# Cython-0.29.tar.gz
wget "https://files.pythonhosted.org/packages/f0/66/6309291b19b498b672817bd237caec787d1b18013ee659f17b1ec5844887/Cython-0.29.tar.gz"
...
wget "https://files.pythonhosted.org/packages/fe/93/b0b203c693d621d6ee9577fc650bab49434c6d23c809dc4eb49db6a339af/numpy-1.19.5-cp37-cp37m-manylinux2014_aarch64.whl"
# psutil-5.8.0.tar.gz
...
sudo yum install -y "${JDK_NAME}" || { echo "Could not install ${JDK_NAME} package"; exit 1;}
...
sudo mkdir -p /data/cassandra/{commitlog,data,saved_caches,hints}/
sudo chmod -R 0700 /data/cassandra

# Copy configurations
sudo cp "${TEMPLATES_DIR}/cassandra.yaml" "${CASSANDRA_ROOT}/conf/cassandra.yaml.template" || exit 1
popd

# 5. Install Django and its dependencies
pushd "${DJANGO_SERVER_ROOT}"

# Create virtual env to run Python 3.7
...
pip3 install "Cython>=0.29,<=0.29.32" --no-index --find-links file://"${DJANGO_WORKLOAD_DEPS}"
