  ...
  if f ~= nil then
    ...
    for line in lines do
      ...
      if fields[1] ~= nil then
        ...
        --                    ["Keep-Alive"]="timeout=60, max=1024"}
...
-- Response callback to print individual requests (similar to Siege)
response = function(status, headers, body)
  ...
  -- Print in simplified format: HTTP/1.1 STATUS BYTES bytes ==> METHOD PATH
  print(string.format("HTTP/1.1 %d   %5d bytes ==> %s  %s",
    status, body_size, method, path))
end
