...
UcacheBenchClient::WarmupResults UcacheBenchClient::warmup() {
  ...
  auto warmupWorker =
          ...
          scope.add(
              folly::coro::co_withExecutor(
                  exe, folly::coro::co_invoke([&]() -> folly::coro::Task<void> {
                    ...
                    if (localOps.load() % 100 == 0) {
                      totalOps.fetch_add(100);
                      setSuccesses.fetch_add(localSuccesses.exchange(0));
                      setErrors.fetch_add(localErrors.exchange(0));
                    }
                    co_return;
                  })));
        }
    ...
    // Final update
    uint64_t remaining = localOps.load() % 100;
    if (remaining > 0) {
      totalOps.fetch_add(remaining);
    }
    setSuccesses.fetch_add(localSuccesses.load());
    setErrors.fetch_add(localErrors.load());
    co_return;
  };
...
UcacheBenchClient::BenchmarkResults UcacheBenchClient::runBenchmark() {
  ...
  if (FLAGS_verbose) {
    progressThread = std::thread([&]() {
      while (!shouldStop.load() && std::chrono::steady_clock::now() < endTime) {
        ...
        auto elapsed = std::chrono::duration<double>(now - startTime).count();
        uint64_t ops = totalOps.load();
        uint64_t gets = getOps.load();
        uint64_t sets = setOps.load();
        uint64_t hits = getHits.load();
        uint64_t misses = getMisses.load();
        uint64_t gErrs = getErrors.load();
        uint64_t sSucc = setSuccesses.load();
        uint64_t sErrs = setErrors.load();
        double avgQps = elapsed > 0 ? ops / elapsed : 0;
  ...
  // Per-worker counters
  std::vector<std::atomic<uint64_t>> workerTotalOps(clients.size());
  std::vector<std::atomic<uint64_t>> workerGetOps(clients.size());
  std::vector<std::atomic<uint64_t>> workerSetOps(clients.size());
  std::vector<std::atomic<uint64_t>> workerGetHits(clients.size());
  std::vector<std::atomic<uint64_t>> workerGetMisses(clients.size());
  std::vector<std::atomic<uint64_t>> workerGetErrors(clients.size());
  std::vector<std::atomic<uint64_t>> workerSetSuccesses(clients.size());
  std::vector<std::atomic<uint64_t>> workerSetErrors(clients.size());
  ...
  auto benchmarkWorker =
    ...
    auto sendGetRequest = [&]() -> folly::coro::Task<void> {
      ...
      workerTotalOps[workerId]++;
      workerGetOps[workerId]++;

      if (*result.result_ref() == carbon::Result::FOUND) {
        workerGetHits[workerId]++;
      } else if (*result.result_ref() == carbon::Result::NOTFOUND) {
        workerGetMisses[workerId]++;
      } else {
        workerGetErrors[workerId]++;
      }
    ...
    auto sendSetRequest = [&]() -> folly::coro::Task<void> {
      ...
      workerTotalOps[workerId]++;
      workerSetOps[workerId]++;

      if (*result.result_ref() == carbon::Result::STORED) {
        workerSetSuccesses[workerId]++;
      } else {
        workerSetErrors[workerId]++;
      }
    ...
    while (std::chrono::steady_clock::now() < endTime) {
        ...
        scope.add(
            folly::coro::co_withExecutor(
                exe, folly::coro::co_invoke([&]() -> folly::coro::Task<void> {
                  ...
                  // Update global counters periodically to avoid atomic
                  // contention
                  if (workerTotalOps[workerId].load() % 100 == 0) {
                    totalOps.fetch_add(100);
                    getOps.fetch_add(workerGetOps[workerId].exchange(0));
                    setOps.fetch_add(workerSetOps[workerId].exchange(0));
                    getHits.fetch_add(workerGetHits[workerId].exchange(0));
                    getMisses.fetch_add(workerGetMisses[workerId].exchange(0));
                    getErrors.fetch_add(workerGetErrors[workerId].exchange(0));
                    setSuccesses.fetch_add(
                        workerSetSuccesses[workerId].exchange(0));
                    setErrors.fetch_add(workerSetErrors[workerId].exchange(0));
                  }
                  co_return;
                })));
      } else {
      ...
      // Rate limiting (per-worker) - only if QPS target is set
      if (FLAGS_qps_target > 0) {
        double workerQpsTarget =
            static_cast<double>(FLAGS_qps_target) / numThreads;
        auto expectedDuration = std::chrono::duration<double>(
            workerTotalOps[workerId].load() / workerQpsTarget);
        auto actualDuration = std::chrono::steady_clock::now() - startTime;

        if (actualDuration < expectedDuration) {
          // Use folly::futures::sleep for non-blocking delay
          co_await folly::futures::sleep(std::chrono::milliseconds(1));
        }
      }

      // Yield to allow other coroutines to run (no delay)
    ...
    // Final update
    uint64_t remaining = workerTotalOps[workerId].load() % 100;
    if (remaining > 0) {
      totalOps.fetch_add(remaining);
    }
    getOps.fetch_add(workerGetOps[workerId].load());
    setOps.fetch_add(workerSetOps[workerId].load());
    getHits.fetch_add(workerGetHits[workerId].load());
    getMisses.fetch_add(workerGetMisses[workerId].load());
    getErrors.fetch_add(workerGetErrors[workerId].load());
    setSuccesses.fetch_add(workerSetSuccesses[workerId].load());
    setErrors.fetch_add(workerSetErrors[workerId].load());
