...
UcacheBenchClient::WarmupResults UcacheBenchClient::warmup() {
  ...
  std::mutex warmupStartMutex;
  std::condition_variable warmupStartCv;
  ...
  auto waitForWarmupStart = [&]() {
    std::unique_lock<std::mutex> lock(warmupStartMutex);
    warmupStartCv.wait(lock, [&]() {
      return warmupStarted.load(std::memory_order_acquire) ||
  ...
  auto warmupThreadGuard = folly::makeGuard([&]() {
    {
      std::lock_guard<std::mutex> lock(warmupStartMutex);
      shouldStop = true;
    }
    warmupStartCv.notify_all();
  ...
  if (!warmupResults.success) {
    throw std::runtime_error(
        "Warmup did not recover to a clean stable tail; refusing to report readiness");
  }
    ...
    auto sendGetRequest =
      ...
      if (sampleLat) {
        ...
        if (cb != 0) {
          ...
          gLatCbSamples.fetch_add(1, std::memory_order_relaxed);
        }
