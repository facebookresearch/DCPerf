...
class SizingTest(unittest.TestCase):
    ...
    def test_proxy_and_density_without_substantial_smt(self) -> None:
        ...
        self.assertEqual(calculate_connection_density(topology), 550)

    def test_proxy_and_density_for_memory_rich_smt(self) -> None:
        topology = HardwareTopology(192, 112, 640 * 1024)

        self.assertEqual(calculate_proxy_count(topology, LoadVariant.PRODUCTION), 24)
        self.assertEqual(calculate_proxy_count(topology, LoadVariant.EXTREME), 56)
        self.assertEqual(calculate_connection_density(topology), 2550)

    def test_proxy_and_density_for_high_core_smt(self) -> None:
        topology = HardwareTopology(152, 92, 384 * 1024)

        self.assertEqual(calculate_proxy_count(topology, LoadVariant.PRODUCTION), 64)
        self.assertEqual(calculate_connection_density(topology), 1500)

    def test_proxy_and_density_for_other_substantial_smt(self) -> None:
        topology = HardwareTopology(88, 48, 192 * 1024)

        self.assertEqual(calculate_proxy_count(topology, LoadVariant.PRODUCTION), 36)
        self.assertEqual(calculate_connection_density(topology), 1400)
    ...
    def test_connections_are_variant_invariant_aligned_and_capped(self) -> None:
        cases = (
            HardwareTopology(119, 80, 192 * 1024),
            HardwareTopology(264, 160, 512 * 1024),
            HardwareTopology(152, 92, 384 * 1024),
            HardwareTopology(88, 48, 192 * 1024),
        )
    ...
    def test_memory_rich_smt_uses_shared_high_capacity_target(self) -> None:
        topology = HardwareTopology(264, 160, 512 * 1024)
        production = calculate_client_shape(topology, LoadVariant.PRODUCTION)
        extreme = calculate_client_shape(topology, LoadVariant.EXTREME)

        self.assertEqual(production.num_proxies, 32)
        self.assertEqual(extreme.num_proxies, 80)
        self.assertEqual(production.total_connections, 407_040)
        self.assertEqual(extreme.total_connections, 407_040)
        self.assertEqual(
    ...
    def test_connections_cap_at_common_aligned_per_process_limit(self) -> None:
        topology = HardwareTopology(20_000, 10_000, 640 * 1024)
        connections = calculate_total_connections(topology)

        self.assertEqual(connections, 2_094_080)
