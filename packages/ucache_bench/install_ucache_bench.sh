...
# Number of parallel jobs for compilation. C++ dependency builds can consume
# more than 1 GiB per compiler process, so keep the default safe on dense hosts.
MAX_NPROC=16
NPROC=${NPROC:-$(nproc)}
case "$NPROC" in
    ''|*[!0-9]*|0)
...
if [ "$NPROC" -gt "$MAX_NPROC" ]; then
    echo "Limiting parallel jobs from $NPROC to $MAX_NPROC"
    NPROC=$MAX_NPROC
fi
...
if ! distro_is_like "ubuntu" && ! distro_is_like "debian" && \
   ! distro_is_like "centos" && ! distro_is_like "rhel" && \
   ! distro_is_like "fedora"; then
...
cmake -S "$CACHELIB_SRC" -B "$CACHELIB_BUILD_DIR" \
    -DCMAKE_INSTALL_PREFIX="$STAGING_DIR" \
    -DCMAKE_PREFIX_PATH="$STAGING_DIR" \
    -DCMAKE_BUILD_TYPE=Release \
    $COMMON_CMAKE_FLAGS \
...
cmake --build "$CACHELIB_BUILD_DIR" --parallel "$NPROC" --target \
    cachelib_common \
    cachelib_shm \
    cachelib_navy \
    cachelib_nvmitem \
...
for t in cachelib_common cachelib_shm cachelib_navy cachelib_nvmitem cachelib_allocator cachelib_datatype; do
    cmake --install "$CACHELIB_BUILD_DIR" --component "$t" 2>/dev/null || true
done
...
cmake -S "$SCRIPT_DIR" -B "$BUILD_DIR" \
    ...
    -DDEPS_DIR="$DEPS_DIR" \
    -DTHRIFT1="$STAGING_DIR/bin/thrift1" \
    -DBUILD_SERVER=ON \
    -DBUILD_CLIENT=ON \
    -DBUILD_TESTING=ON
...
cmake --build "$BUILD_DIR" --parallel "$NPROC" --target \
    ucachebench_server \
    ucachebench_client \
    ucachebench_bind_source

ctest --test-dir "$BUILD_DIR" \
    -R '^ucachebench_oss_smoke$' \
    --output-on-failure
cmake --install "$BUILD_DIR"
...
install -D -m 0755 \
    "$INSTALL_DIR/bin/ucachebench_server" \
    "$UCACHE_BENCH_DIR/server/ucachebench_server"
install -D -m 0755 \
    "$INSTALL_DIR/bin/ucachebench_client" \
    "$UCACHE_BENCH_DIR/client/ucachebench_client"
install -D -m 0755 \
    "$INSTALL_DIR/lib/bind_source.so" \
    "$UCACHE_BENCH_DIR/client/bind_source.so"
echo "  Installed: $UCACHE_BENCH_DIR/server/ucachebench_server"
echo "  Installed: $UCACHE_BENCH_DIR/client/ucachebench_client"
echo "  Installed: $UCACHE_BENCH_DIR/client/bind_source.so"
...
echo "  ./benchpress install ucache_bench_memory"
echo "  ./benchpress info ucache_bench_memory"
echo ""
