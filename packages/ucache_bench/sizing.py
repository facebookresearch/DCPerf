...
def calculate_connection_density(topology: HardwareTopology) -> int:
    if not topology.substantial_smt:
        return 550
    if topology.memory_mib >= _MEMORY_RICH_MIB:
        return 2550
...
def calculate_total_connections(topology: HardwareTopology) -> int:
    ...
    if max_connections < common_quantum:
        raise ValueError("client topologies have no common connection allocation")

    target = topology.physical_cores * calculate_connection_density(topology)
    rounded = max(
        common_quantum,
        _round_to_nearest_multiple(target, common_quantum),
    )
    return min(max_connections, rounded)
