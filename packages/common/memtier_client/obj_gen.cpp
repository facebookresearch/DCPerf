...
void random_generator::set_seed(int seed)
{
    ...
    // Compiler warning with @mode/opt if assert is used
    // assert(initstate_r(seed, m_state_array, sizeof(m_state_array), &m_data_blob) == 0);
...
void object_generator::alloc_value_buffer()
{
    ...
    if (size > 0) {
        ...
        if (!m_random_data) {
            ...
            unsigned int i = 0;
            unsigned int x0 = (unsigned int)(crnt_time % 20200413);
            unsigned int x1 = 0;
