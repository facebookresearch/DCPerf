...
abstract_protocol::abstract_protocol() :
    m_read_buf(nullptr), m_write_buf(nullptr), m_keep_value(false)
{
}
...
protocol_response::protocol_response()
    : m_status(nullptr), m_mbulk_value(nullptr), m_value(nullptr), m_value_len(0), m_hits(0), m_error(false)
{
}
...
bool protocol_response::is_error()
{
    return m_error;
}
...
const char* protocol_response::get_status()
{
    return m_status;
}
...
unsigned int protocol_response::get_total_len()
{
    return m_total_len;
}

void protocol_response::incr_hits()
{
    m_hits++;
}

unsigned int protocol_response::get_hits()
{
    return m_hits;
}

void protocol_response::clear()
{
    if (m_status != nullptr) {
        free((void *)m_status);
        m_status = nullptr;
    ...
    if (m_mbulk_value != nullptr) {
        delete m_mbulk_value;
        m_mbulk_value = nullptr;
    }
...
public:
    ...
    virtual redis_protocol* clone() { return new redis_protocol(); }
...
int redis_protocol::parse_response()
{
    char *line;
    size_t res_len;
    int ret = -1;
    ...
    while (true) {
        switch (m_response_state) {
            ...
            case rs_read_line:
                ...
                if (line[0] == '*') {
                    int count = strtol(line + 1, nullptr, 10);
                    ...
                    if (m_keep_value) {
                        ...
                        if (m_last_response.get_mbulk_value() == nullptr) {
                    ...
                    int len = strtol(line + 1, nullptr, 10);
                    m_last_response.set_status(line);
...
public:
    ...
    virtual memcache_text_protocol* clone() { return new memcache_text_protocol(); }
    virtual int select_db(int db);
    virtual int authenticate(const char *credentials);
    virtual int write_command_cluster_slots();
    virtual int write_command_set(const char *key, int key_len, const char *value, int value_len, int expiry, unsigned int offset);
...
int memcache_text_protocol::parse_response()
{
    char *line;
    size_t tmplen;
    int ret = -1;
    ...
    while (true) {
        switch (m_response_state) {
            ...
            case rs_read_section:
                ...
                if (m_last_response.get_status() == nullptr) {
                    m_last_response.set_status(line);
                }
            ...
            case rs_read_value:
                if (evbuffer_get_length(m_read_buf) >= m_value_len + 2) {
                    if (m_keep_value) {
                        ...
                        // Compiler warning with @mode/opt
                        ret = evbuffer_remove(m_read_buf, value, m_value_len);
                        assert(ret == 0);

                        m_last_response.set_value(value, m_value_len);
                    } else {
                        // Compiler warning with @mode/opt
                        ret = evbuffer_drain(m_read_buf, m_value_len);
                        assert(ret == 0);
                    }
...
public:
    ...
    virtual memcache_binary_protocol* clone() { return new memcache_binary_protocol(); }
    virtual int select_db(int db);
    virtual int authenticate(const char *credentials);
    virtual int write_command_cluster_slots();
    virtual int write_command_set(const char *key, int key_len, const char *value, int value_len, int expiry, unsigned int offset);
...
const char* memcache_binary_protocol::status_text()
{
    int status;
...
int memcache_binary_protocol::parse_response()
{
    while (true) {
        int ret;
        int status;
...
keylist::keylist(unsigned int max_keys) :
    m_buffer(nullptr), m_buffer_ptr(nullptr), m_buffer_size(0),
    m_keys(nullptr), m_keys_size(0), m_keys_count(0)
{
...
unsigned int keylist::get_keys_count() const
{
    return m_keys_count;
}
