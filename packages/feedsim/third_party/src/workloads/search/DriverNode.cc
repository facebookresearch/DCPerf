...
struct ThreadData {
  ...
  uint64_t search_request_delay;  // This is per thread
...
void RecomputeDelayTimerHandler(evutil_socket_t listener, int16_t flags,
  ...
  double qps =
      static_cast<double>(stats.query_counts_.at(search::kSearchRequestType)) /
      (stats.end_time_ - stats.start_time_) * 1000000000;

  // Adjust delay based on QPS
  this_thread->search_request_delay *= (qps / this_thread->qps_per_thread);
...
void ThreadStartup(oldisim::NodeThread& thread,
  ...
  if (args.qps_arg != 0) {
    this_thread.qps_per_thread = (static_cast<double>(args.qps_arg)) /
        args.threads_arg;
    this_thread.recompute_qps_timer = evtimer_new(
        thread.get_event_base(), RecomputeDelayTimerHandler, &this_thread);
    AddRecomputeDelayTimer(this_thread);
    this_thread.search_request_delay = 1000000 / this_thread.qps_per_thread;
  } else {
    this_thread.search_request_delay = 0;
  }
}
...
void MakeRequest(oldisim::NodeThread& thread, oldisim::TestDriver& test_driver,
  ...
  test_driver.SendRequest(search::kSearchRequestType,
                          this_thread.random_string.c_str(),
                          3000, this_thread.search_request_delay);
}
...
int main(int argc, char** argv) {
  ...
  std::string hostname;
  int port;
  search::ParseServerAddress(args.server_arg, hostname, port);

  // Make storage for thread variables
  std::vector<ThreadData> thread_data(args.threads_arg);

  oldisim::DriverNode driver_node(hostname, port);
  ...
  driver_node.SetMakeRequestCallback(
      std::bind(MakeRequest, std::placeholders::_1, std::placeholders::_2,
                std::ref(thread_data)));
  driver_node.RegisterRequestType(search::kSearchRequestType);
